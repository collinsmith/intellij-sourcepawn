...
import net.alliedmods.lang.sourcepawn.util.TemplateUtils;
...
public class SpModuleBuilder extends ModuleBuilder implements SourcePathsBuilder {
  ...
  public void setupRootModel(ModifiableRootModel rootModel) throws ConfigurationException {
    if (sdk == null) {
      Collection<Sdk> sdks = SpSdkType.getSpSdks();
      if (!sdks.isEmpty()) {
        sdk = sdks.stream().findFirst().orElse(null);
        ...
        if (sources != null) {
          for (Pair<String, String> p : sources) {
            ...
            if (sourceDir != null) {
              ...
              String name = rootModel.getModule().getName() + "." + SpFileType.INSTANCE.getDefaultExtension();
              TemplateUtils.createOrResetFileContentFromTemplate(sourceDir, name, SpSupport.PLUGIN_TEMPLATE);
            }
          }
        }
      }
  ...
  public ModuleType getModuleType() {
    return SpModuleType.getInstance();
  }
  ...
  public boolean isSuitableSdkType(SdkTypeId sdkType) {
    return sdkType == SpSdkType.getInstance();
  }
